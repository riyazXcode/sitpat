  ...
  return (
      <aside className={`h-screen ${isCollapsed ? 'w-12' : 'w-64'} transition-all duration-200`}>
        <nav className="h-full flex flex-col border-r shadow-sm bg-bg-grey">
          ...
          <h1 className="text-white text-2xl pl-4 pr-4">Student Database</h1>
          <ul className="ml-3 mt-2">
            {["Add Student Details", "Delete Student Details", "Update Student Details", "View Student Database"].map((item) => (
                <li
                    key={item}
                    onClick={() => handleItemClick(item)}
                    className="group pl-4 hover:text-black hover:bg-white py-2 transition duration-500 hover:cursor-pointer hover:border-l-8 hover:border-main-blue"
                >
          ...
          <hr className="my-3"></hr>
          <h1 className="text-white text-2xl pl-4 pr-4">Placement Database</h1>
          <ul className="ml-3 mt-2">
            {["Add Placement Training", "Delete Placement Training", "Update Placement Training", "View Placement Database"].map((item) => (
                <li
                    key={item}
                    onClick={() => handleItemClick(item)}
                    className="group pl-4 hover:text-black hover:bg-white py-2 transition duration-500 hover:cursor-pointer hover:border-l-8 hover:border-main-cyan-green"
                >
                  <span className=" text-white group-hover:text-black transition duration-200">{item}</span>
                </li>
            ))}
